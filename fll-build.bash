	...
	if [ "$FLL_DISTRO_CODENAME" = "snapshot" ]; then
		if [[ ${#FLL_BUILD_ARCH[@]} -gt 1 ]]; then
			FLL_ISO_NAME=$(tr A-Z a-z <<< \
				${FLL_DISTRO_NAME}-${FLL_PACKAGE_TIMESTAMP}-${FLL_DISTRO_CODENAME_SAFE}-${FLL_BUILD_PACKAGE_PROFILE}-multiarch.iso)
		else
			FLL_ISO_NAME=$(tr A-Z a-z <<< \
		...
		if [[ ${#FLL_BUILD_ARCH[@]} -gt 1 ]]; then
			FLL_ISO_NAME=$(tr A-Z a-z <<< \
				${FLL_DISTRO_NAME}-${FLL_DISTRO_VERSION}-${FLL_PACKAGE_TIMESTAMP}-${FLL_DISTRO_CODENAME_SAFE}-${FLL_BUILD_PACKAGE_PROFILE}-multiarch.iso)
		else
			FLL_ISO_NAME=$(tr A-Z a-z <<< \
