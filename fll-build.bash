...
for config in ${FLL_BUILD_CONFIGS[@]}; do
	...
	sed -i	-e 's#^id:[0-6]:initdefault:#id:5:initdefault:#' \
		...
		-e 's#^\(1\):\([0-9]\+\):\(respawn\):.\+#\1:\2:\3:/bin/bash --login --rcfile /root/.bashrc >/dev/tty\1 2>\&1 </dev/tty\1#' \
		-e 's#^\([2-6]\):\([0-9]\+\):\(respawn\):.\+#\1:\245:\3:/bin/bash --login --rcfile /root/.bashrc >/dev/tty\1 2>\&1 </dev/tty\1#' \
